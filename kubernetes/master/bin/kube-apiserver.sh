...
echo "Create ${KUBE_CFG_DIR}/token_auth_file.csv"
export BOOTSTRAP_TOKEN=$(head -c 16 /dev/urandom | od -An -t x | tr -d ' ')
cat	<<EOF >${KUBE_CFG_DIR}/token_auth_file.csv
${BOOTSTRAP_TOKEN},kubelet-bootstrap,10001,"system:kubelet-bootstrap"
EOF

echo "Create ${KUBE_CFG_DIR}/basic_auth_file.csv"
cat	<<EOF >${KUBE_CFG_DIR}/basic_auth_file.csv
admin,admin,1
system,system,2
EOF
...
- name: controllermanager
  user:
    client-certificate: ${KUBE_SSL_DIR}/cs_client.crt
    client-key: ${KUBE_SSL_DIR}/cs_client.key
clusters:
- name: local
  cluster:
    certificate-authority: ${KUBE_SSL_DIR}/ca.crt
contexts:
- context:
    cluster: local
...
KUBE_API_CLIENT_CA_FILE="--client-ca-file=${KUBE_SSL_DIR}/ca.crt"

# --service-account-key-file="":服务账号文件，包含x509公私钥
KUBE_SERVICE_ACCOUNT_KEY_FILE="--service-account-key-file=${KUBE_SSL_DIR}/ca.key"
...
KUBE_API_TLS_CERT_FILE="--tls-cert-file=${KUBE_SSL_DIR}/server.crt"

# --tls-private-key-file="": File containing x509 private key matching --tls-cert-file.
KUBE_API_TLS_PRIVATE_KEY_FILE="--tls-private-key-file=${KUBE_SSL_DIR}/server.key"
...
KUBE_TOKEN_AUTH_FILE="--token-auth-file=${KUBE_CFG_DIR}/token_auth_file.csv"

#--basic-auth-file=/srv/kubernetes/basic_auth_file.csv
KUBE_BASIC_AUTH_FILE="--basic-auth-file=${KUBE_CFG_DIR}/basic_auth_file.csv"
...
echo "Create ${KUBE_SERVICE}/kube-apiserver.service file"
cat <<EOF >${KUBE_SERVICE}/kube-apiserver.service
[Unit]
