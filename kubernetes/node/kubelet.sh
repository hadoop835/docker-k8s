...
export NODE_ADDRESS=192.168.0.108
export KUBELET_POD_INFRA_CONTAINER=hub.c.163.com/k8s163/pause-amd64:3.0
...
- name: kubelet
  user:
    client-certificate: ${KUBE_SSL_DIR}/kube_client.crt
    client-key: ${KUBE_SSL_DIR}/kube_client.key
clusters:
- name: local
  cluster:
...
echo "Create ${KUBE_SERVICE}/kubelet.service file"
cat <<EOF >${KUBE_SERVICE}/kubelet.service
[Unit]
